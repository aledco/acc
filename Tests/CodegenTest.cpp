...
TEST(Codegen, All)
{
    for (auto& input : all_inputs)
    {
        ...
        execute(input.test_id, "clang++-18 " + output_files.s_filepath + " -o " + output_files.exe_filepath);
        ...
        execute(input.test_id, "clang-18 " + input.filepath + " -o " + output_files.ref_exe_filepath);
